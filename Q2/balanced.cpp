...
/*String will all closed brackets*/
const string CLOSE = ")]}";
...
bool is_balanced(const string& expression){
    ...
    while (balanced && (iter != expression.end())){
        char next_char = *iter;
        if (is_open(next_char)){ //if func is_open, push char onto stack
            st1.push(next_char);
        } else if (is_close(next_char)){ //else, checks func is_close
            if (st1.empty()){   //if stack is empty, balanced is F
                balanced = false;
            } else{ //else, pop stack
                char top_ch = st1.top();
                st1.pop();
